...
class ActionSearchRestaurants(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		Restaurant_Search_results = RestaurantSearch(City=loc,Cuisine=cuisine)
		budget= tracker.get_slot('price')
		results=budget(budget,Restaurant_Search_results)

		response=""
		if results.shape[0] == 0:
			response= "Oops, Can't find a "+cuisine+" Restaurant in "+loc+" for a budget "+budget
		else:
			for restaurant in budget(budget,Restaurant_Search_results).iloc[:5].iterrows():
...
class ActionSendMail(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		if results.shape[0] == 0:
			...
			for restaurant in budget(budget,Restaurant_Search_results).iloc[:10].iterrows():
				restaurant = restaurant[1]
		...
		try:
			...
			server.starttls()
			server.login("", "") #please use credentials

			message = MIMEMultipart()
			message['From'] = "xxxx@gmaiil.com"
			message['To'] = MailID
			message['Subject'] = 'Your Restaurant Search Details! :)'

			body = "Check out the Top 10 restuarants that match you requirements:-\n\n"+response
			message.attach(MIMEText(body,"plain"))
			text = message.as_string()
			server.sendmail("", MailID, text)
			print("Mail Sent!")
			server.quit()	      
